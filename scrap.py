...
def extract_posts_from_html(html: str, db_connection: sqlite3.Connection) -> List[tuple[str, str, str]]:
    ...
    print(f"║ Posts Count: {len(post_wrappers)}" + " " * (120 - 16 - len(str(len(post_wrappers)))) + "║")
    print("╚" + "═" * 118 + "╝")
    ...
    for post_wrapper in post_wrappers:
        ...
        if not post_container:
            ...
            continue
        raw_text = post_container.decode_contents()  # type: ignore
        if not raw_text.strip():
            # print("    Warning: Empty post.")
            continue

        id = utils.generate_post_id(raw_date, raw_text)
        ...
        try:
            ...
            print(f"┃ Raw date:\t{raw_date}" + " " * (120 - 17 - len(raw_date)) + "┃")
            print(f"┃ Clean Date:\t{clean_date}" + " " * 83 + "┃")
        except Exception as e:
            utils.print_error_and_exit(f"Error parsing date: '{raw_date}'", e)

        print("┠" + "─" * 118 + "┨")
        utils.print_boxed_text(raw_text, 120)
        # clean_post = utils.clean_post_text(raw_text)
        # print("┠" + "─" * 118 + "┨")
        # print(f"┃ {clean_post}")
        print("┗" + "━" * 118 + "┛")

        posts.append((id, clean_date, raw_text))
...
def save_posts(db_connection: sqlite3.Connection, posts: List[tuple[str, str, str]]) -> None:
    cursor = db_connection.cursor()
    cursor.executemany("INSERT OR IGNORE INTO posts (id, date, raw_text) VALUES (?, ?, ?)", posts)
...
def main() -> None:
    ...
    try:
        ...
        while page <= 500:
            print("\n╔" + "═" * 118 + "╗")
            print(f"║ PAGE {page}" + " " * (120 - 8 - len(str(page))) + "║")
            print("╟" + "─" * 118 + "╢")

            page_html = driver.page_source
            posts = extract_posts_from_html(page_html, db_connection)
